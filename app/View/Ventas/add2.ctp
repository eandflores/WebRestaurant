<form method="post" class="form-horizontal well" action="/furaibar/Ventas/add3/">
	<fieldset>
		<h3>Productos</h3>
				...
				if($Datos_productos!=null){
				    foreach ($Datos_productos as $key => $value) { ?>
			    		<tr>
			      			...
			      			<td>
				      			<input id="<?php echo 'producto'.$key ?>" name="cantidadproducto[]" type="number" min="0" value="0" class="producto">
				      		</td>
				...
				if($Datos_promocion!=null){
				    foreach ($Datos_promocion as $key => $value){ ?>
				    	<tr>
			      			...
			      			<td>
				      			<input id="<?php echo 'promocion'.$key ?>" name="cantidadpromocion[]" type="number" min="0" value="0" class="producto">
				      		</td>
	    ...
	    <div>
	        <div class="controls" style="float:left; margin-left:30px; margin-top:20px;">
	           <input id="total" type="text" min="0" value="0" readonly style="color:black; width:150px;">
	        </div>
	    </div>
	    <select style="float:left; margin-left:20px; margin-top:20px; width:100px;" name="mesa">
	    	<?php 
	    		foreach ($_mesas as $key => $mesa) { ?>
	    			<option value="<?php echo $mesa->mes_id; ?>">
	    				<?php echo "Mesa ".$mesa->mes_numero; ?> 
	    			</option>
	    	<?php
	    		}
	    	?>
		...
		<div class="form-actions">
	        <button class="btn btn-primary" type="submit" style="margin-left:20px; width:150px;">Solicitar Pedido</button>
	        <button class="btn" type="reset" onclick="window.location='/furaibar/Deusuario'" style="margin-left:15px; width:150px;">Cancelar</button>
        </div>
    </fieldset>
</form>
<script>
